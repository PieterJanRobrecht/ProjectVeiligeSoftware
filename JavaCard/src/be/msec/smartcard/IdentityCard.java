...
public class IdentityCard extends Applet {
	...
	public void process(APDU apdu) throws ISOException {
		...
		case SIGN_INS:
			signSomething(apdu);
			break;
		case LENGTH_CERT_INS:
			certificateLength(apdu);
			break;
	...
	private void certificateLength(APDU apdu){
		byte[] certLength = intToByteArray(certificate.length);

		apdu.setOutgoing();
		apdu.setOutgoingLength((short) certLength.length);
		apdu.sendBytesLong(certLength,(short)0,(short)certLength.length);
	}
	...
	private void signSomething(APDU apdu) {		
		short offset = 0;
		...
		short sigLength = signature.sign(input, ISO7816.OFFSET_CDATA, length, output,(short) 0);
