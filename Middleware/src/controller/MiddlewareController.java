...
public class MiddlewareController {
	...
	public byte[] authenticateCard(byte[] challenge) {
		...
		try {
			...
			if (r.getSW() == KAPPA) {
				System.out.println("Signature made on the card");
			} else if (r.getSW() != 0x9000)
				throw new Exception("Exception on the card: " + Integer.toHexString(r.getSW()));
			...
			if (isSimulator) {
				part1 = slice(part1, 6, part1.length);
			}
			...
			if (isSimulator) {
				part2 = slice(part2, 6, part2.length);
			}
			...
			if (isSimulator) {
				part3 = slice(part3, 6, part3.length);
			}
	...
	public byte[] requestReleaseOfAttributes(byte[] resp) {
		...
		try {
			...
			/** fetch answer from req to JC part 1 **/
			a = new CommandAPDU(IDENTITY_CARD_CLA, FETCH_REQ_ATT_INS, (byte) 0, 0x00, 0xff);
			r = connection.transmit(a);
			...
			byte[] inc = r.getData();
			System.out.println("\tPayload query1: " + Arrays.toString(inc));
			
			for(int i = 0; i < 250; i++) {
				response[i] = inc[i];
			}
				
			/** fetch answer from req to JC part 2 **/
			a = new CommandAPDU(IDENTITY_CARD_CLA, FETCH_REQ_ATT_INS, (byte) 1, 0x00, 0xff);
			r = connection.transmit(a);
			...
			inc = r.getData();
			System.out.println("\tPayload query2: " + Arrays.toString(inc));
	...
	public byte[] createFakeCertificate(X509Certificate cert) {
		...
		/** type **/
		temp = new ArrayList<Byte>();
		valseKappa = cert.getSubjectDN().toString().split("CN=")[1].getBytes();
		for (int i = 0; i < 4; i++) {
			temp.add(valseKappa[i]);
		}
		list.addAll(temp);
